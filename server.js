// ==========================================================
// server.js — DISABLED (site is fully static on Vercel)
// War game multiplayer runs via api/socket.js (serverless)
// To re-enable local dev: uncomment everything below
...
//       roundWinner:  roundWinner === null ? 'tie' : roundWinner === idx ? 'you' : 'opponent',
...
//     const room = { code, players: [{ socket, name, hand: [], score: 0, played: null }], round: 1, totalRounds: 4, state: 'waiting' };
//     rooms.set(code, room);
//     socket.join(code);
...
//       player.socket.emit('game-start', { hand: player.hand, opponentName: room.players[1 - idx].name, round: room.round, totalRounds: room.totalRounds, playerIndex: idx });
//     });
//   });
...
//     if (room.players[0].played && room.players[1].played) _resolveRound(room);
//   });
...
//     const room = rooms.get(code);
//     if (!room) return;
//     room.players.forEach(p => { if (p.socket.id !== socket.id) p.socket.emit('opponent-disconnected'); });
//     rooms.delete(code);
//   });
// });
//
